export const projects = [
    {
        id: "motoshop",
        title: "Linq CRM",
        description: "Full system for managing motorcycle parts sales, inventory, POS, purchases, suppliers, users, permissions, cash registers, interest calculations, mixed payments and more.",
        tech: ["Django", "Python", "SQLite", "MySQL", "Bootstrap"],
        link: "https://github.com/PaulGerman23/motoshop",
        github: "https://github.com/PaulGerman23/motoshop",
        image: "/images/LinCRM.png",
        category: "complete-apps"
    },
    {
        id: "robotics",
        title: "RoboticsFixture",
        description: "Match generator for robotics competitions with categories, match creation, standings, repechaje, and automated bracket logic.",
        tech: [".NET MVC", "SQL Server"],
        link: "https://github.com/PaulGerman23",
        github: "https://github.com/PaulGerman23",
        image: "/images/Fixture.png",
        category: "complete-apps"
    },
    {
        id: "explorer",
        title: "Explorador de Salta",
        description: "Mobile game developed in Flutter with SQLite. Includes categories, game logic, animations and question banks.",
        tech: ["Flutter", "Dart", "SQLite"],
        link: "https://github.com/PaulGerman23",
        github: "https://github.com/PaulGerman23",
        image: "/images/charades.jpg",
        category: "complete-apps",
        isVertical: true
    },
    {
        id: "inventory",
        title: "Sistema de Inventario COPROTAB",
        description: "Inventory control system built during professional practice, managing product stock, categories, movements and reports.",
        tech: ["Python", "Tkinter", "SQLite"],
        link: "#",
        github: "#",
        image: "/images/cliente_creditos.png",
        category: "complete-apps"
    }
];
